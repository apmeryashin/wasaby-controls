...
export default class Explorer extends Control<IExplorerOptions> {
    ...
    // tslint:disable-next-line:no-any - нужно разобраться с тем какой тут точно интерфейс лучше задекларировать
    protected _navigation: INavigationOptionValue<any>;
    ...
    /**
     * Промис, который существует на время загрузки дополнительных модулей (tile, columnsView).
     */
    private _additionalModuleLoadPromise: Promise<unknown>;
    //endregion
    ...
    protected _beforeMount(cfg: IExplorerOptions): Promise<void> | void {
        ...
        this._onSourceControllerDataLoad = this._onSourceControllerDataLoad.bind(this);
    ...
    protected _afterMount(): void {
        ...
        /**
         * Если есть sourceController, то подписываемся на его событие dataLoad для того, что бы
         * задержать обработку новых данных на время пока у нас не загрузятся дополнительные модули.
         * Иначе возникнут не желательные перерисовки.
         */
        if (this._options.sourceController) {
            this._options.sourceController.subscribe('dataLoad', this._onSourceControllerDataLoad);
        }
    }
    ...
    protected _beforeUnmount(): void {
        ...
        if (this._options.sourceController) {
            this._options.sourceController.unsubscribe('dataLoad', this._onSourceControllerDataLoad);
        }
    }
    ...
    protected _documentDragStart(event: SyntheticEvent, dragObject: IDragObject<ItemsEntity>): void {
        ...
        if (
            ...
            // tslint:disable-next-line:ban-ts-ignore
            // @ts-ignore - не полностью описан интерфейс у dragObject.entity
    ...
    private _setViewMode(viewMode: TExplorerViewMode, cfg: IExplorerOptions): Promise<void> | void {
        ...
        if (action instanceof Promise) {
            this._additionalModuleLoadPromise = action;
            return action.then(() => {
                this._setViewModeSync(viewMode, cfg);
                this._additionalModuleLoadPromise = null;
            });
        }
    ...
    private _onSourceControllerDataLoad(event: EventObject): void {
        event.setResult(this._additionalModuleLoadPromise);
    }
