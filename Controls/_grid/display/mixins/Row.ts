...
export default abstract class Row<T extends Model = Model> {
    ...
    getStickyHeaderPosition(stickyCallback: Function): {} {
        ...
        const stickyHorizontalPosition = this.hasNewColumnScroll() ? 'left' : undefined;
    ...
    hasStickyColumns(): boolean {
        return this.hasNewColumnScroll() && this.isFullGridSupport() && this.getStickyColumnsCount() !== 0;
    }
    ...
    protected _prepareColumnItems(columns: IColspanParams[],
        ...
        if (creatingColumnsParams.length === 1 && (this._$rowTemplate || columns.length > 1)) {
            ...
            if (this.hasColumnScroll() || this.hasNewColumnScroll()) {
                creatingColumnsParams[0].isFixed = true;
            }
    ...
    hasNewColumnScroll(): boolean {
        return this._$owner.hasNewColumnScroll();
    }
