...
const GridView = ListView.extend([ColumnScrollViewMixin], {
    ...
    _getLadderTopOffsetStyles(): string {
        ...
        return `.${this._ladderOffsetSelector} .${ladderClass} {` +
                  `top: calc(var(--item_line-height_l_grid) + ${offsetFromOptions} + ${headerHeight + resultsHeight}px) !important;}` +
                `.${this._ladderOffsetSelector} .${ladderClass}_withGroup {` +
                   `top: calc(var(--item_line-height_l_grid) + var(--grouping_height_list) + ${offsetFromOptions} + ${headerHeight + resultsHeight}px) !important;}`;
