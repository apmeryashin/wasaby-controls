...
export interface IInsertExtraItemIndexParams <S extends Model = Model, T extends TreeItem<S> = TreeItem<S>> {
    extraItem: T;
    extraItems: T[];
    itemsOrder: number[];
    items: T[];
...
export default abstract class ExtraNodeItem<S extends Model = Model, T extends TreeItem<S> = TreeItem<S>>
    ...
    private _removeExtraItems(removedItems: T[]): void {
        removedItems.forEach((item) => {
            const index = this._extraItems.findIndex((extraItem: T) => extraItem.getNode() === item);
            if (index !== -1) {
    ...
    protected _getSortItemsOptions(): ISortOptions<S, T> {
        return {
            ...
            extraItemModule: this.options.extraItemModule,
            extraItems: this._extraItems,
            extraItemVisibilityCallback: this.options.extraItemVisibilityCallback
        };
    }
    ...
    protected _sortItems(items: T[], options: ISortOptions<S, T>): number[] {
        const extraItemsContent = options.extraItems.map((it) => it.getContents());
        ...
        const newExtraItemsContent = nodesWithExtraItem.map((extraItem) =>
            this._getExtraItemContent(extraItem));
        ...
        extraItemsContent.forEach((extraItemContent) => {
            if (newExtraItemsContent.indexOf(extraItemContent) === -1) {
                const index = options.extraItems.findIndex((it) => it.contents === extraItemContent);
        ...
        const itemsOrder = items.map((it, index) => index + countExtraItems);
    ...
    protected _getNodesWithExtraItem(items: T[], options: ISortOptions<S, T>): T[] {
        ...
        for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
            ...
            if (
                !item['[Controls/_display/TreeItem]'] || item[`[${options.extraItemModule}]`]
    ...
    protected _updateParentInExtraItems(items: TreeItem[], extraItems: TreeItem[]): void {
        extraItems.forEach((extraItem: T) => {
            ...
            const newNode = items.find((it) => it.key === nodeKey) as T;
            if (newNode) {
