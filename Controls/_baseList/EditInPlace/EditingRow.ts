...
export default class EditingRow extends Control {
    ...
    protected _onKeyDown(event): void {
        ...
        if (
            !(event.nativeEvent.keyCode === constants.key.enter && this._options.keepEnter) && (
    ...
    protected _stopMouseEvent(e, eventName): void {
        /*
         * Останавливаем всплытие любых кликов, если строка редактируется.
         * Если клики будут всплывать, то их будет ловить список
         ...
         * - На каждом компоненте, который будет лежать внутри редактируемой строки,
         * придется останавливать всплытие.
