...
export class TreeSelectionStrategy implements ISelectionStrategy {
   ...
   private _getBreadcrumbsSelected(item: BreadcrumbsItem, selection: ISelection): boolean|null {
      ...
      if (this._isAllSelectedInRoot(selection)) {
         ...
         // Если в хлебной кршке выбранный ключем является не последний ключ - это значит что сперва выбрали узел и
         // потом только начали искать. В этом случае рисуем квадрат, т.к. мы не можем точно гарантировать,
         // что выбраны все дети хлебной крошки.
