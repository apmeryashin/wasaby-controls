 ...
 */
export function split(oldValue: string, newValue: string, caretPosition: number,
                      selection, inputType: InputType): ISplitValue {
   ...
   const insertedValue: string = newValue.substring(beforeInsertedValue.length,
                                                   newValue.length - afterInsertedValue.length);
   const deletedValue: string = oldValue.substring(beforeInsertedValue.length,
...
function isCorrectlySplit(splitValue: ISplitValue, oldValue: string, newValue: string): boolean {
   return splitValue.before + splitValue.delete + splitValue.after === oldValue &&
