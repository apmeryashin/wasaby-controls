...
export class HierarchicalReverseMemory extends HierarchicalMemory {
    query(query?: Query): Promise<DataSet> {
        ...
        const direction =
            where.hasOwnProperty(keyProperty + '~') ? 'bothways' :
            where.hasOwnProperty(keyProperty + '>=') ? 'forward' : 'backward';
        const position =
            direction === 'bothways' ? where[keyProperty + '~'] :
            direction === 'forward' ? where[keyProperty + '>='] : where[keyProperty + '<='];
        const isRootLoad = parents === null || parents.length === 1 && parents[0] === null;

        if (isRootLoad) {
            const rootItems = this.data.filter((item) => item[parentProperty] === null);
        ...
        return Promise.resolve(
            this._selectItemsFromRoot(limit, parents, parentProperty, keyProperty, direction, position)
        );
    }

    _selectItemsFromRoot(limit: number, rootKey: number, parentProperty: string,
                         keyProperty: string, direction: string, position: number): DataSet {
        const items = [];
        const rootItems = this.data.filter((item) => item[parentProperty] === rootKey).reverse();
        let index = 0;
        if (position !== null) {
            index = rootItems.findIndex((item) => {
