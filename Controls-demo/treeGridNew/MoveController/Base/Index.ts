...
import {ItemActionsHelpers} from 'Controls/list';
...
export default class extends Control {
   ...
   protected _itemActionVisibilityCallback(action: IItemAction, item: Model): boolean {
      ...
      if (action.id === 'moveUp' || action.id === 'moveDown') {
         direction = action.id === 'moveUp' ?
             ItemActionsHelpers.MOVE_DIRECTION.UP :
             ItemActionsHelpers.MOVE_DIRECTION.DOWN;
         result = ItemActionsHelpers
             .reorderMoveActionsVisibility(direction, item, this._items, this._parentProperty, this._nodeProperty);
      }
