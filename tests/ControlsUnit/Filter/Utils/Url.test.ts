...
describe('Controls/_filter/Utils/Url', () => {
    ...
    it('getQueryParamsByFilter', () => {
        const queryParams = getQueryParamsByFilter([
            ...
            {name: 'testName', value: [{value: 'testValue'}], textValue: 'testText', resetValue: null, visibility: true}
        ]);
        const expected = {filter: '[{"name":"testName","value":"testValue","textValue":"testText","visibility":' +
    ...
    it('updateUrlByFilter', () => {
        ...
        assert.deepEqual(queryParams, {filter: '[{"name":"testName","value":"testValue",' +
                '"textValue":"testText","visibility":true}]'});
        assert.deepEqual(state, '/?filter=%5B%7B%22name%22%3A%22testName%22%2C%22value%22%3A%22testValue' +
            '%22%2C%22textValue%22%3A%22testText%22%2C%22visibility%22%3Atrue%7D%5D');
    });

    it('getFilterFromUrl', () => {
        const urlFilter = {filter: '%5B%7B%22name%22%3A%22Organization%22%2C%22value%22%3A21391705%2C%22textValue' +
                '%22%3A%22%D0%9D%D0%9E%D0%92%D0%AB%D0%99%20%D0%A2%D0%95%D0%A1%D0%A2%22%7D%5D'};
        sinon.stub(query, 'get').value(urlFilter);

        assert.deepEqual(getFilterFromUrl(), [{
            name: 'Organization',
            value: 21391705,
